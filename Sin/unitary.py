...
logsPath = "./GraphUnitary"

tf.reset_default_graph()
x = tf.placeholder(tf.float32, [None,1], name="Features")
y = tf.placeholder(tf.float32, [None,1], name="Labels")

model = tf.layers.Dense(1024, activation=tf.nn.relu, name="1_dense")(x)
model = tf.layers.Dense(512, activation=tf.nn.relu, name="2_dense")(model)
